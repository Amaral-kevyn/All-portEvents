...
require_once dirname(__FILE__).'/../Models/department.php';
    class users
    {
        ...
        public function __construct($_users_id =0,$_lastname='',$_firstname='',$_birthdate='',$_email='',$_pseudo='',$_password='',$_photo='',$_civility='',$_actif=0,$_token='',$_dateOfCreation='',$_admin_id=65498,$_department_id=0)
        {
		...
		{
			$insertUser = 'INSERT INTO `users`(`users_id`,`lastname`,`firstname`,`birthdate`,`email`,`pseudo`,`password`,`photo`,`civility`,`actif`,`token`,`admin_id`,`department_id`) VALUES (:users_id, :lastname, :firstname, :birthdate, :email, :pseudo, :password, :photo, :civility, :actif, :token, :admin_id, :department_id)';
            ...
            $usersStatement->bindvalue(':department_id',$this->department_id,PDO::PARAM_INT);
        ...
        public function readSingle()
		{
			...
			$sql_viewUsers = 'SELECT `users_id`,`photo`,`lastname`, `firstname`,DATE_FORMAT(`birthdate`,"%d/%m/%Y") AS birthdate_format,`birthdate`,`email`,`pseudo`,`civility`,`admin_id`,`civility`,`dateOfCreation`,`departmentName`,`departmentCode`,users.department_id FROM `users` JOIN `department` ON 
            users.department_id = department.department_id WHERE `users_id` = :users_id';
        ...
        public function readPregMatchPseudo()
        {
            $users_sqlPseudo = 'SELECT `pseudo` FROM `users` WHERE `pseudo`=:pseudo';
            $userStatmentPseudo = $this->db->prepare($users_sqlPseudo);
            $userStatmentPseudo->bindValue(':pseudo', $this->pseudo, PDO::PARAM_STR);
            $usersPseudo = null;
            if ($userStatmentPseudo->execute()) {
                $usersPseudo = $userStatmentPseudo->fetchAll(PDO::FETCH_OBJ);
            }
            return $usersPseudo;
        }
        ...
        public function readAll()
		{
            ...
            $usersList_sql = 'SELECT `users_id`,`admin_id`,`photo`,`lastname`,`department_id`,`civility`, `firstname`,TIMESTAMPDIFF(year,birthdate,CURRENT_DATE) AS age, `pseudo` FROM `users` ORDER BY `lastname` ASC';
        ...
        {
            $sqlUpdate = 'UPDATE users SET lastname=:lastname,firstname=:firstname,photo=:photo,birthdate=:birthdate,email=:email,pseudo=:pseudo,civility=:civility,department_id=:department_id WHERE users_id=:users_id';
